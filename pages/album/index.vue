	...
	<view>
		...
		<view class="album_author">
			...
			<view class="album_author_desc">
			    <text>{{album.desc}}</text><!-- 需text包裹，desc中的“\n”才能生效 -->
			</view>
		</view>
		<!--专辑作者结束-->
		...
		data() {
			return {
				params: {
					...
					order:'new',
					skip:0,
					//1 返回值中有album对象（第一次请求数据）
					//0 返回值中只有wallpaper数组（不是第一次）
					first:1
				},
				id:-1,
				album:{},
		...
		methods: {
			getList() {
				this.request({
					...
					console.log(result);
					// console.log(JSON.stringify(result.res.album.desc));//发现有的desc中含“\n”
					if(Object.keys(this.album).length === 0){//如果album是空对象
